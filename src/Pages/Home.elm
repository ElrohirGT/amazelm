...
type alias Model =
    Result
        ...
        , carrousel : CarrouselModel
        , keepBuying : KeepBuyingModel
...
init =
    let
        capitalizeString s =
            String.concat [ String.toUpper (String.left 1 s), String.slice 1 (String.length s) s ]

        keepBuyingMapper : Product -> BoughtCategories
        keepBuyingMapper product =
            { name = capitalizeString product.category
    ...
    in
    Result.map responseMapper (decodeString getProductsResponseDecoder sampleResponse)
        ...
        TickCarrousel ->
            let
                mapper model =
                    ...
                    in
                    { model
                        | carrousel = { carrousel | currentIndex = modBy (Array.length carrousel.products) (carrousel.currentIndex + 1) }
                    }
            in
            ( Result.map mapper resModel, Cmd.none )

        MoveCarrouselIndex idx ->
            let
                mapper model =
                    let
                        carrousel =
                    ...
                    in
                    { model
                        | carrousel = { carrousel | currentIndex = idx }
                    }
            in
            ( Result.map mapper resModel, Cmd.none )
...
subscriptions _ =
    Time.every 3000 (\_ -> TickCarrousel)
...
keepBuyingView : KeepBuyingModel -> Html Msg
keepBuyingView model =
    div
        [ css
            [ padding cssGaps.s ]
        ]
        ...
        , div
            ...
            (displayKeepBuyingCategories model)
        ]


displayKeepBuyingCategories : KeepBuyingModel -> List (Html Msg)
displayKeepBuyingCategories model =
    case model.categories of
        ...
        current :: rest ->
            List.concat
                ...
                , displayKeepBuyingCategories { model | categories = rest }
                ]


carrouselView : CarrouselModel -> Html Msg
carrouselView model =
    let
        maybeProduct =
    ...
    let
        ...
        elem =
            button
                ...
                , onClick (MoveCarrouselIndex idx)
                ]
                []
    in
